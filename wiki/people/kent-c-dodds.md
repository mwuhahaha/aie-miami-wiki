---
title: Kent C. Dodds
category: people
aliases:
  - Kent C. Dodds
sourceLabels:
  - Transcript-derived
  - Conference website reference
last_auto_summarized: '2026-04-25T06:56:36.392Z'
---
# Kent C. Dodds

## Summary

Kent C. Dodds is represented in this public AI Engineer Miami 2026 wiki as a conference participant with source-bound links to the public event context and the transcript corpus. The biographical framing stays conservative: it records the linked session information and the themes visible in the local conference map without adding private biographical material.

## Contribution at AI Engineer Miami

Kent C. Dodds's linked session was [[talks/2026-04-20-kent-c-dodds-build-a-free-agent|Build a Free Agent]]. The talk page carries the session-level context and source labels.

## Talks

- [[talks/2026-04-20-kent-c-dodds-build-a-free-agent|Build a Free Agent]] (2026-04-20)

## Related Themes

- [[topics/personal-agents|Personal Agents]], [[topics/agent-workflows|Agent Workflows]]

## Sources

- **Transcript-derived:** `/tmp/aie-miami-transcript.txt and /tmp/aie-miami-part2-transcript.txt`.
- **Conference website reference:** https://www.ai.engineer/miami
- **Public-web supporting context:** no additional public-web supporting context is cited on this page.
